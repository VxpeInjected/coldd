-- Checkout "add to your order" suggestions. Previously this only returned
-- products that shared a *keyword-detected genre* with something in the
-- cart, so with most of the catalogue detecting no genre at all it came
-- back empty and the section never showed. Now it always ranks the whole
-- on-platform catalogue and lets relevance (genre > category > subcategory
-- > same platform) dominate the ordering, with the existing
-- quality/price/revenue conversion score only breaking ties inside a
-- relevance band.
--
-- It also now returns the exact deal price the checkout will actually
-- honour: 10% off, but never below product_legal.min_sale_usd, and no
-- discount at all when product_legal.disallow_sales is set (the item can
-- still be suggested, just at list price). The client shows precisely
-- this number, so the "was / now" it displays is always the real price
  ...
  priced as (
    select c.*,
      ...
      case
        ...
        else greatest(round(c.price_usd * 0.90, 2), coalesce(c.min_sale_usd, 0))
